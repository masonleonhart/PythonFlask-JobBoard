...
def employer(employer_id):
    ...
    employer_obj = execute_sql('SELECT * FROM employer WHERE id=?', [employer_id], single=True)
    jobs_list = execute_sql('SELECT job.id, job.title, job.description, job.salary FROM job JOIN employer ON employer.id = job.employer_id WHERE employer.id = ?', [employer_id])
    reviews_list = execute_sql('SELECT review, rating, title, date, status FROM review JOIN employer ON employer.id = review.employer_id WHERE employer.id = ?', [employer_id])
